...
const renderDeliveryHours = (deliveryWeek: Week, c_specific_day: any) => {
  const dayDom: JSX.Element[] = [];
  let i = 0;
...
const Hours = (props: Hours) => {
  ...
  const [open2, setOpen2] = React.useState("hidden");
  const [hoursopen, setHoursOpen] = React.useState(true);
  const [hoursopen2, setHoursOpen2] = React.useState(true);
  function join(t: any, a: any, s: any) {
    function format(m: any) {
      const f = new Intl.DateTimeFormat("en", m);
      return f.format(t);
  ...
  return (
    <>
      {deliveryHours ? (
        ...
        <>
          {document?.c_hoursHeading ? (
            <h4 className="box-title">{document?.c_hoursHeading}</h4>
          ) : (
            ""
          )}
          <table className="day-hrs">
